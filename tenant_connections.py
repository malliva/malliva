...
class connect_to_database:
    ...
    def initiate_db_connection():
        ...
        mongoengine.register_connection(alias=self.alias, db=self.database, host=self.host, username=self.username, password=self.password)

    def switch_database(new_database):
        #mongoengine.context_managers.switch_db()

    def switch_collection(new_database):
